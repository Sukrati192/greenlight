...
func (app *application) logError(c *gin.Context, err error) {
	app.logger.PrintError(err, map[string]string{"request_method": c.Request.Method, "request_url": c.Request.URL.String()})
}

func (app *application) errorResponse(c *gin.Context, status int, message interface{}) {
	data := envelope{"error": message}
	if err := app.writeJSON(c, status, data, nil); err != nil {
		app.logError(c, err)
...
func (app *application) invalidAuthenticationResponse(c *gin.Context) {
